...
export default async function CoursesPage() {
    ...
    return (
      <main style={{ padding: '2rem', maxWidth: 640, margin: '0 auto' }}>
        ...
        <p style={{ color: '#94a3b8' }}>
          Sign in with email and password (and institution slug on localhost) so your session includes an API access
          token.
        </p>
  ...
  if (!canRead) {
    return (
      <main style={{ padding: '2rem', maxWidth: 640, margin: '0 auto' }}>
        ...
        <h1 style={{ color: '#f8fafc' }}>Courses</h1>
        <p style={{ color: '#94a3b8' }}>
          Your account does not have the <strong>lms.read</strong> permission. Ask an administrator to add LMS access
          to your role.
        </p>
      </main>
    );
  ...
  const res = await fetch(`${apiBase}/lms/course-instances?limit=48`, {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      >
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
          <span style={{ color: '#38bdf8', fontWeight: 600 }}>My courses</span>
          <Link href="/dashboard" style={{ color: '#94a3b8', textDecoration: 'none', fontSize: '0.9rem' }}>
            Dashboard
          </Link>
          <Link href="/students" style={{ color: '#94a3b8', textDecoration: 'none', fontSize: '0.9rem' }}>
            Students
          </Link>
        </nav>
        {canWrite ? (
          <p style={{ fontSize: '0.75rem', color: '#64748b', marginTop: 'auto' }}>
            Faculty: use <Link href="/teach" style={{ color: '#38bdf8' }}>Teach</Link> for builder shortcuts (preview).
          </p>
        ) : null}
      </aside>
      <main style={{ flex: 1, padding: '2rem 1.75rem', maxWidth: 1100 }}>
        ...
        <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.95rem' }}>
          {payload.total} course shell{payload.total === 1 ? '' : 's'} linked to your sections · LMS Phase 4.3
        </p>
        {payload.data.length === 0 ? (
          <p style={{ color: '#64748b' }}>
            No LMS course instances yet. With <strong>lms.write</strong>, create one via{' '}
            <code style={{ fontSize: '0.85rem' }}>POST /lms/course-instances</code> using a section id from Academic.
          </p>
        ) : (
          <div
            style={{
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                    <h2 style={{ margin: 0, fontSize: '1.05rem', color: '#0f1729', fontWeight: 700 }}>
                      {c.course.code}
                    </h2>
                    <span
                      style={{
                  ...
                  <p style={{ margin: '0.35rem 0 0', fontSize: '0.88rem', color: '#475569', lineHeight: 1.35 }}>
                    {c.course.title}
                  </p>
                  <p style={{ margin: 'auto 0 0', fontSize: '0.8rem', color: '#94a3b8' }}>{c.semester.name}</p>
                </Link>
                {canWrite ? (
                  <div
                    style={{
                  ...
                  >
                    <Link href={`/teach/${c.id}`} style={{ color: '#2563eb', fontWeight: 600, textDecoration: 'none' }}>
                      Teach (preview)
                    </Link>
                  </div>
                ) : null}

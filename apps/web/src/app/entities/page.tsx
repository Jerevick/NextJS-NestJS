...
import { EntitiesDashboard, type ConsolidatedStatsPayload, type EntityListRow } from '@/components/entities/entities-dashboard';
...
export default async function EntitiesPage() {
  ...
  const canCreate = perms.includes('*') || perms.includes('institutions.write');
  ...
  const statsRes =
    canManageAll && listRes.ok
      ? await fetch(`${apiBase}/institutions/${session.user.institutionId}/entities/consolidated/stats`, {
          headers,
          cache: 'no-store',
        })
      : null;
  ...
  return (
    <main style={{ padding: '2rem', fontFamily: 'system-ui', maxWidth: 1040 }}>
      ...
      <p style={{ color: '#64748b', fontSize: '0.95rem' }}>
        Active context: {session.user.entityId || '—'} · scope <strong>{session.user.entityScope}</strong>
      </p>
      <EntitySwitcher
      ...
      />
      <EntitiesDashboard rows={rows} stats={stats} canManageAll={canManageAll} canCreate={canCreate} />

...
function mockContext(req: { user?: AuthUser; body?: unknown; params?: Record<string, string> }): ExecutionContext {
  return {
    switchToHttp: () => ({
      getRequest: () => req,
    }),
...
describe('StudentRecordPostingGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new StudentRecordPostingGuard(prisma as unknown as PrismaService, reflector);
  });
  ...
  it('allows INACTIVE student when an active backfill window covers the record date', async () => {
    ...
    const req: { user?: AuthUser; body?: unknown; params?: Record<string, string>; backfillContext?: unknown } = {
      user: actor,
